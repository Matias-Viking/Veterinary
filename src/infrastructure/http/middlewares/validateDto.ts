...
import { IllegalArgumentError, InvalidParameter } from "../../../application/errors/IllegalArgumentError";
import { plainToInstance } from "class-transformer";
import { validate } from "class-validator";



function mapValidationErrorToInvalidParamter(error:any,isArray:boolean,index?:number):InvalidParameter{
    ...
    const recurse = (error:any,parentPath?:string):InvalidParameter =>{
        ...
        const children = error && Array.isArray(error.children) && error.children.length>0 ? error.children.map((child:any)=> recurse(child,currentPath)) : undefined;
...
function processValidationResults(results:any[],isArray:boolean):InvalidParameter[]{
    ...
    for(const result of results){
        ...
        if(result.errors.length>0){
            ...
            const invalidParameters=result.errors.map((error:any)=>mapValidationErrorToInvalidParamter(error,isArray,result.index));
...
export function validateRequestBody<T extends object>(cls: new ()=>T):RequestHandler{
    ...
    return async function(req:Request,_res:Response,next:NextFunction){
        ...
        try {
            ...
            const dtos=bodyData.map((item:any)=>plainToInstance(cls,item,{enableImplicitConversion:true}),);
            ...
            if (allErrors.length>0){
             return next(new IllegalArgumentError(allErrors,req.body as object));
            }
